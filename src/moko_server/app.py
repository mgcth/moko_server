    ...
    with open(CAMERA_LIST_FILE, "r") as file:
        try:
            ...
            for camera_name, data in data.items():
                ...
                fps = data["fps"]
                camera_manager.camera = camera_manager.selected(
                    path, camera_name, resolution_id, rotation, quality, fps
                )
...
async def camera_config(request):
    ...
    if camera_manager.selected is None:
        return empty
...
async def stream(request, ws):
    ...
    try:
        while True:
            ...
            await ws.send(f"data:image/jpeg;base64, {base64.b64encode(frame).decode()}")
